...
from queries import CREATE_TABLE_QUERY, DROP_TABLE_QUERY
from queries import QUERY_OVER_60YO_WITH_GMAIL, QUERY_PERCENTAGE_DE_USERS_WITH_GMAIL, QUERY_TOP_3_COUNTRIES_WITH_GMAIL
...
if __name__=="__main__":
    ...
    logger.info(f"Creating table {SQLITE_TABLE_NAME} in SQLite DB {SQLITE_DB_NAME}...")
    execute_in_sqlite3(SQLITE_DB_NAME, DROP_TABLE_QUERY.format(table_name=SQLITE_TABLE_NAME), display=False)
    execute_in_sqlite3(SQLITE_DB_NAME, CREATE_TABLE_QUERY.format(table_name=SQLITE_TABLE_NAME), display=False)


    logger.info(f"Writing {len(df_persons_address_age_grouped)} records of API data to table {SQLITE_TABLE_NAME}...")
    conn = sqlite3.connect(SQLITE_DB_NAME)
    df_persons_address_age_grouped_email.to_sql(SQLITE_TABLE_NAME, con=conn, if_exists='replace', index=False)


    logger.info(f"Analysis: Which percentage of users live in Germany and use Gmail as an email provider?")
    print(pd.read_sql(QUERY_PERCENTAGE_DE_USERS_WITH_GMAIL, conn))


    logger.info("Analysis: Which are the top three countries in our database that use Gmail as an email?")
    print(pd.read_sql(QUERY_TOP_3_COUNTRIES_WITH_GMAIL, conn))


    logger.info("Analysis: How many people over 60 years use Gmail as an email provider?")
    print(pd.read_sql(QUERY_OVER_60YO_WITH_GMAIL, conn))

    conn.close()
    logger.info("Thank you for your attention. Chao!")
